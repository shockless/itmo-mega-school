...
SEARCH_TOPIC_PROMPT = """Please provide up to 2 necessary keywords related to your research topic for Google search. 4. Use only official ITMO University URLs, do not use wikipedia!!. Use ITMO.ru and all subdomains. USE ONLY AND JUST official ITMO University URLs, like https://itmo.ru and all its subdomains like https://news.itmo.ru and others
USE ONLY OFFICIAL WEBSITE OF ITMO UNIVERSITY!
Always use the word "Университет ИТМО" in the search query!
Ensure that the search query is complete and contains all necessary information.
Every part of the query is important!!!
 \
...
4. Use only official ITMO University URLs, do not use wikipedia!! Use ITMO.ru and all subdomains

USE ONLY OFFICIAL WEBSITE OF ITMO UNIVERSITY!

USE ONLY AND JUST official ITMO University URLs, like https://itmo.ru and all its subdomains like https://news.itmo.ru and others
...
5. Pay attention to all the information provided in the reference information, such as dates, numbers, etc.
6. Fact check the information provided in the reference information, such as dates, numbers, etc.
7. USE ONLY AND JUST official ITMO University URLs, like https://itmo.ru and all its subdomains like https://news.itmo.ru and others
8. USE ONLY OFFICIAL WEBSITE OF ITMO UNIVERSITY!
...
- The report should have a minimum word count of 1000 and be formatted with Markdown syntax following APA style guidelines.
- Pay attention to all the information provided in the reference information, such as dates, numbers, etc.
- Fact check the information provided in the reference information, such as dates, numbers, etc.
"""
...
class CollectLinks(Action):
    ...
    async def run(
        ...
        prompt = reduce_message_length(
            gen_msg(), model_name, system_text, config.llm.max_token
        )
        logger.debug(prompt)
        ...
        for query in queries:
            ret[query] = await self._search_and_rank_urls(
                topic, query, url_per_query, max_results
            )
        return ret

    async def _search_and_rank_urls(
        self, topic: str, query: str, num_results: int = 4, max_results: int = 6
    ) -> list[str]:
        ...
        """
        max_results = max(num_results * 2, max_results)
