...
from ..actions.research import (
    Action,
    CollectLinks,
    ConductResearch,
    WebBrowseAndSummarize,
)
from ..actions.research import get_research_system_text
...
class ITMOResearcher(Role):
    ...
    async def _act(self) -> Message:
        ...
        if isinstance(todo, CollectLinks):
            links = await todo.run(topic, self.decomposition_nums, self.url_per_query)
            ret = Message(
                content="",
            ...
            urls = [url for i in summaries for (url, _) in i.items()]
            summaries = list(
                (url, summary)
                for i in summaries
            ...
            ret = Message(
                content="",
                instruct_content=Report(topic=topic, summaries=summaries, urls=urls),
                role=self.profile,
                cause_by=todo,
            )
        else:
            summaries = instruct_content.summaries
            urls = instruct_content.urls
            seen = set()
            unique_ordered = []
            for url in urls:
                if url not in seen:
                    unique_ordered.append(url)
                    seen.add(url)
            ...
            ret = Message(
                ...
                instruct_content=Report(
                    topic=topic, content=content, urls=unique_ordered
                ),
                role=self.profile,
...
if __name__ == "__main__":
    ...
    async def main(topic: str, language: str = "ru", enable_concurrency: bool = True):
        role = ITMOResearcher(language=language, enable_concurrency=enable_concurrency)
